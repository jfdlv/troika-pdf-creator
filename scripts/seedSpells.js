...
// Troika! Numinous Edition — Spells (Chapter 14)
// To cast a Spell you spend Stamina equal to the casting cost (the number in
// brackets) and Roll Under your Skill Total in that Spell. A double 1 always
// succeeds; a double 6 always fails and requires a roll on the Oops! Table.
//
// `cost` is the Stamina cost. It is a Number where the rules give a flat value.
// Spells with conditional/unusual costs keep a numeric `cost` where one exists
// (e.g. Illusion's per-viewer base of 2) plus a `costNote`, or `cost: null`
// where the cost is undefined (Undo, Zed).
...
const troikaSpells = [
  ...
  },
  {
    ...
    // Damage by Damage Roll (1–7+): 3, 3, 5, 7, 9, 12, 16
    damage: { 1: 3, 2: 3, 3: 5, 4: 7, 5: 9, 6: 12, "7+": 16 },
  },
  {
    name: "Flash",
    cost: 3,
  ...
  },
  {
    ...
    // Damage by Damage Roll (1–7+): 2, 2, 3, 3, 5, 7, 9
    damage: { 1: 2, 2: 2, 3: 3, 4: 3, 5: 5, 6: 7, "7+": 9 },
  },
  {
    name: "Languages",
    cost: 1,
  ...
  },
  {
    name: "Undo",
    cost: null,
    costNote: "double cost of original Spell",
    description:
...
const seed = async () => {
  const col = db.collection('spells');
  const batch = db.batch();

  for (const spell of troikaSpells) {
    const ref = col.doc(toDocId(spell.name));
    batch.set(ref, spell, { merge: true });
  }

  await batch.commit();
  console.log(`Seeded ${troikaSpells.length} spells into the "spells" collection.`);
};
